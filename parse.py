...
def compute_simiarity_features(questions, answers, model, threshold, q_max_sent_length, a_max_sent_length):
    print "Checking dim of questions and answers"
    print len(questions)
    print len(answers)

    q_res, a_res = [], []
    for q, a in zip(questions, answers):
        q_arr = np.zeros(q_max_sent_length)
        for x in xrange(len(q)):

            max_score = 0
            for y in xrange(len(a)):
                try:
                    score = model.similarity(q[x],a[y])
                    if score > threshold:
        ...
        a_arr = np.zeros(a_max_sent_length)
        for x in xrange(len(a)):
            arr = np.zeros(a_max_sent_length)
            max_score = 0
            for y in xrange(len(q)):
                try:
                    score = model.similarity(q[y],a[x])
                    if score > threshold:
                        a_arr[x] = 1
                except KeyError:
                    pass
    ...
    q_sim = np.vstack(q_res).astype('int32')
    a_sim = np.vstack(a_res).astype('int32')
...
if __name__ == '__main__':
  ...
  train = os.path.join(folder_name, 'train.xml')
  train_files = [train]

  for train in train_files:
    print train

    dev = os.path.join(folder_name, 'dev.xml')
    test = os.path.join(folder_name, 'test.xml')

    outdir = os.path.join(folder_name, "TRAIN")
    print 'outdir', outdir
    ...
    for fname in [train, dev, test]:
      ...
      q_sim, a_sim = compute_simiarity_features(questions, answers, model, 0.8, q_max_sent_length, a_max_sent_length)
      ...
      np.save(os.path.join(outdir, '{}.q_sim.npy'.format(basename)), q_sim)
      np.save(os.path.join(outdir, '{}.a_sim.npy'.format(basename)), a_sim)
